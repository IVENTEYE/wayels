...
const Shipments = () => {
  ...
  const renderItems = useTypedSelector((state: TypeRootState) => state.shipments.renderItems);
  ...
  useEffect(() => {
    const handleElement: IShipment[] = renderItems.filter((item: IShipment) => item.id === activeTruckId);
  ...
  return (
    <div className='mb-4 mt-[65px] tablet:mt-0'>
      <div className='flex items-center max-w-7xl'>
        <h1 className="font-bold flex-auto text-xl text-[#232137] my-7">Shipments</h1>
      </div>
      <div className="shipments relative flex flex-col mobile-md:grid mobile-md:grid-cols-cards gap-[10px]">
        ...
        {packagesTransition((style, item) => (
          item &&
          <animated.div className="h-[665px]" style={style}>
            <Packages>
              {packages.map((item) => {
                return (
                  <div
